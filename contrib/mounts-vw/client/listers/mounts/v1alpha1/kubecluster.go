...
type KubeClusterClusterLister interface {
	...
	List(selector labels.Selector) (ret []*mountsv1alpha1.KubeCluster, err error)
...
func (s *kubeClusterClusterLister) List(selector labels.Selector) (ret []*mountsv1alpha1.KubeCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mountsv1alpha1.KubeCluster))
	})
	return ret, err
}
...
type KubeClusterLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*mountsv1alpha1.KubeCluster, error)
...
func (s *kubeClusterLister) List(selector labels.Selector) (ret []*mountsv1alpha1.KubeCluster, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*mountsv1alpha1.KubeCluster))
	})
	return ret, err
}

// Get retrieves the KubeCluster from the indexer for a given workspace and name.
func (s *kubeClusterLister) Get(name string) (*mountsv1alpha1.KubeCluster, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(mountsv1alpha1.Resource("kubeclusters"), name)
	}
	return obj.(*mountsv1alpha1.KubeCluster), nil
}
...
// List lists all KubeClusters in the indexer for a workspace.
func (s *kubeClusterScopedLister) List(selector labels.Selector) (ret []*mountsv1alpha1.KubeCluster, err error) {
...
// Get retrieves the KubeCluster from the indexer for a given workspace and name.
func (s *kubeClusterScopedLister) Get(name string) (*mountsv1alpha1.KubeCluster, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
