...
type cronJobClusterLister struct {
	kcplisters.ResourceClusterIndexer[*batchv1.CronJob]
}
...
func NewCronJobClusterLister(indexer cache.Indexer) CronJobClusterLister {
	return &cronJobClusterLister{
		kcplisters.NewCluster[*batchv1.CronJob](indexer, batchv1.Resource("cronjob")),
	}
}

// Cluster scopes the lister to one workspace, allowing users to list and get CronJobs.
func (l *cronJobClusterLister) Cluster(clusterName logicalcluster.Name) listersbatchv1.CronJobLister {
...
// or scope down to a listersbatchv1.CronJobNamespaceLister for one namespace.
type cronJobLister struct {
	kcplisters.ResourceIndexer[*batchv1.CronJob]
}

var _ listersbatchv1.CronJobLister = new(cronJobLister)

// CronJobs returns an object that can list and get CronJobs in one namespace.
func (l *cronJobLister) CronJobs(namespace string) listersbatchv1.CronJobNamespaceLister {
	return &cronJobNamespaceLister{
		l.ResourceIndexer.WithNamespace(namespace),
	}
}

// cronJobNamespaceLister implements the listersbatchv1.CronJobNamespaceLister
// interface.
type cronJobNamespaceLister struct {
	kcplisters.ResourceIndexer[*batchv1.CronJob]
}

var _ listersbatchv1.CronJobNamespaceLister = new(cronJobNamespaceLister)
...
func NewCronJobLister(indexer cache.Indexer) listersbatchv1.CronJobLister {
	return &cronJobLister{
		kcplisters.New[*batchv1.CronJob](indexer, batchv1.Resource("cronjob")),
	}
}
...
// CronJobs returns an object that can list and get CronJobs in one namespace.
func (l *cronJobScopedLister) CronJobs(namespace string) listersbatchv1.CronJobLister {
	return &cronJobLister{
		l.ResourceIndexer.WithNamespace(namespace),
	}
}
