...
type ResourceClaimClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) resourcev1beta1listers.ResourceClaimLister
...
func (s *resourceClaimClusterLister) List(selector labels.Selector) (ret []*resourcev1beta1.ResourceClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*resourcev1beta1.ResourceClaim))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get ResourceClaims.
func (s *resourceClaimClusterLister) Cluster(clusterName logicalcluster.Name) resourcev1beta1listers.ResourceClaimLister {
	return &resourceClaimLister{indexer: s.indexer, clusterName: clusterName}
}

// resourceClaimLister implements the resourcev1beta1listers.ResourceClaimLister interface.
...
func (s *resourceClaimLister) List(selector labels.Selector) (ret []*resourcev1beta1.ResourceClaim, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*resourcev1beta1.ResourceClaim))
	})
	return ret, err
}

// ResourceClaims returns an object that can list and get ResourceClaims in one namespace.
func (s *resourceClaimLister) ResourceClaims(namespace string) resourcev1beta1listers.ResourceClaimNamespaceLister {
	return &resourceClaimNamespaceLister{indexer: s.indexer, clusterName: s.clusterName, namespace: namespace}
}

// resourceClaimNamespaceLister implements the resourcev1beta1listers.ResourceClaimNamespaceLister interface.
...
func (s *resourceClaimNamespaceLister) List(selector labels.Selector) (ret []*resourcev1beta1.ResourceClaim, err error) {
...
func (s *resourceClaimNamespaceLister) Get(name string) (*resourcev1beta1.ResourceClaim, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(resourcev1beta1.Resource("resourceclaims"), name)
	}
	return obj.(*resourcev1beta1.ResourceClaim), nil
}
