...
type DeviceClassClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) resourcev1beta1listers.DeviceClassLister
...
func (s *deviceClassClusterLister) List(selector labels.Selector) (ret []*resourcev1beta1.DeviceClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*resourcev1beta1.DeviceClass))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get DeviceClasses.
func (s *deviceClassClusterLister) Cluster(clusterName logicalcluster.Name) resourcev1beta1listers.DeviceClassLister {
	return &deviceClassLister{indexer: s.indexer, clusterName: clusterName}
}

// deviceClassLister implements the resourcev1beta1listers.DeviceClassLister interface.
...
func (s *deviceClassLister) List(selector labels.Selector) (ret []*resourcev1beta1.DeviceClass, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*resourcev1beta1.DeviceClass))
	})
	return ret, err
}

// Get retrieves the DeviceClass from the indexer for a given workspace and name.
func (s *deviceClassLister) Get(name string) (*resourcev1beta1.DeviceClass, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(resourcev1beta1.Resource("deviceclasses"), name)
	}
	return obj.(*resourcev1beta1.DeviceClass), nil
}
