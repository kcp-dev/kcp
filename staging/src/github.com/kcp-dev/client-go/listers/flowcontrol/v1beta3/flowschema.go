...
type FlowSchemaClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) flowcontrolv1beta3listers.FlowSchemaLister
...
func (s *flowSchemaClusterLister) List(selector labels.Selector) (ret []*flowcontrolv1beta3.FlowSchema, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*flowcontrolv1beta3.FlowSchema))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get FlowSchemas.
func (s *flowSchemaClusterLister) Cluster(clusterName logicalcluster.Name) flowcontrolv1beta3listers.FlowSchemaLister {
	return &flowSchemaLister{indexer: s.indexer, clusterName: clusterName}
}

// flowSchemaLister implements the flowcontrolv1beta3listers.FlowSchemaLister interface.
...
func (s *flowSchemaLister) List(selector labels.Selector) (ret []*flowcontrolv1beta3.FlowSchema, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*flowcontrolv1beta3.FlowSchema))
	})
	return ret, err
}

// Get retrieves the FlowSchema from the indexer for a given workspace and name.
func (s *flowSchemaLister) Get(name string) (*flowcontrolv1beta3.FlowSchema, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(flowcontrolv1beta3.Resource("flowschemas"), name)
	}
	return obj.(*flowcontrolv1beta3.FlowSchema), nil
}
