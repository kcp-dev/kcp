...
type PriorityLevelConfigurationClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) flowcontrolv1listers.PriorityLevelConfigurationLister
...
func (s *priorityLevelConfigurationClusterLister) List(selector labels.Selector) (ret []*flowcontrolv1.PriorityLevelConfiguration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*flowcontrolv1.PriorityLevelConfiguration))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get PriorityLevelConfigurations.
func (s *priorityLevelConfigurationClusterLister) Cluster(clusterName logicalcluster.Name) flowcontrolv1listers.PriorityLevelConfigurationLister {
	return &priorityLevelConfigurationLister{indexer: s.indexer, clusterName: clusterName}
}

// priorityLevelConfigurationLister implements the flowcontrolv1listers.PriorityLevelConfigurationLister interface.
...
func (s *priorityLevelConfigurationLister) List(selector labels.Selector) (ret []*flowcontrolv1.PriorityLevelConfiguration, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*flowcontrolv1.PriorityLevelConfiguration))
	})
	return ret, err
}

// Get retrieves the PriorityLevelConfiguration from the indexer for a given workspace and name.
func (s *priorityLevelConfigurationLister) Get(name string) (*flowcontrolv1.PriorityLevelConfiguration, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(flowcontrolv1.Resource("prioritylevelconfigurations"), name)
	}
	return obj.(*flowcontrolv1.PriorityLevelConfiguration), nil
}
