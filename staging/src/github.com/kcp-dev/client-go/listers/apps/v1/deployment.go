...
type deploymentClusterLister struct {
	kcplisters.ResourceClusterIndexer[*appsv1.Deployment]
}
...
func NewDeploymentClusterLister(indexer cache.Indexer) DeploymentClusterLister {
	return &deploymentClusterLister{
		kcplisters.NewCluster[*appsv1.Deployment](indexer, appsv1.Resource("deployment")),
	}
}

// Cluster scopes the lister to one workspace, allowing users to list and get Deployments.
func (l *deploymentClusterLister) Cluster(clusterName logicalcluster.Name) listersappsv1.DeploymentLister {
...
// or scope down to a listersappsv1.DeploymentNamespaceLister for one namespace.
type deploymentLister struct {
	kcplisters.ResourceIndexer[*appsv1.Deployment]
}

var _ listersappsv1.DeploymentLister = new(deploymentLister)

// Deployments returns an object that can list and get Deployments in one namespace.
func (l *deploymentLister) Deployments(namespace string) listersappsv1.DeploymentNamespaceLister {
	return &deploymentNamespaceLister{
		l.ResourceIndexer.WithNamespace(namespace),
	}
}

// deploymentNamespaceLister implements the listersappsv1.DeploymentNamespaceLister
// interface.
type deploymentNamespaceLister struct {
	kcplisters.ResourceIndexer[*appsv1.Deployment]
}

var _ listersappsv1.DeploymentNamespaceLister = new(deploymentNamespaceLister)
...
func NewDeploymentLister(indexer cache.Indexer) listersappsv1.DeploymentLister {
	return &deploymentLister{
		kcplisters.New[*appsv1.Deployment](indexer, appsv1.Resource("deployment")),
	}
}
...
// Deployments returns an object that can list and get Deployments in one namespace.
func (l *deploymentScopedLister) Deployments(namespace string) listersappsv1.DeploymentLister {
	return &deploymentLister{
		l.ResourceIndexer.WithNamespace(namespace),
	}
}
