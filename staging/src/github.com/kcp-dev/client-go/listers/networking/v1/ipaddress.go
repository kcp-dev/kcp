...
type iPAddressClusterLister struct {
	kcplisters.ResourceClusterIndexer[*networkingv1.IPAddress]
}
...
func NewIPAddressClusterLister(indexer cache.Indexer) IPAddressClusterLister {
	return &iPAddressClusterLister{
		kcplisters.NewCluster[*networkingv1.IPAddress](indexer, networkingv1.Resource("ipaddress")),
	}
}

// Cluster scopes the lister to one workspace, allowing users to list and get IPAddresses.
func (l *iPAddressClusterLister) Cluster(clusterName logicalcluster.Name) listersnetworkingv1.IPAddressLister {
...
// or scope down to a listersnetworkingv1.IPAddressNamespaceLister for one namespace.
type iPAddressLister struct {
	kcplisters.ResourceIndexer[*networkingv1.IPAddress]
}

var _ listersnetworkingv1.IPAddressLister = new(iPAddressLister)
...
func NewIPAddressLister(indexer cache.Indexer) listersnetworkingv1.IPAddressLister {
	return &iPAddressLister{
		kcplisters.New[*networkingv1.IPAddress](indexer, networkingv1.Resource("ipaddress")),
	}
}

// iPAddressScopedLister can list all IPAddresses inside a workspace
// or scope down to a listersnetworkingv1.IPAddressNamespaceLister.
