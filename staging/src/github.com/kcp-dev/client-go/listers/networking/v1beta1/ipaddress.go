...
type IPAddressClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) networkingv1beta1listers.IPAddressLister
...
func (s *iPAddressClusterLister) List(selector labels.Selector) (ret []*networkingv1beta1.IPAddress, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networkingv1beta1.IPAddress))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get IPAddresses.
func (s *iPAddressClusterLister) Cluster(clusterName logicalcluster.Name) networkingv1beta1listers.IPAddressLister {
	return &iPAddressLister{indexer: s.indexer, clusterName: clusterName}
}

// iPAddressLister implements the networkingv1beta1listers.IPAddressLister interface.
...
func (s *iPAddressLister) List(selector labels.Selector) (ret []*networkingv1beta1.IPAddress, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*networkingv1beta1.IPAddress))
	})
	return ret, err
}

// Get retrieves the IPAddress from the indexer for a given workspace and name.
func (s *iPAddressLister) Get(name string) (*networkingv1beta1.IPAddress, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(networkingv1beta1.Resource("ipaddresses"), name)
	}
	return obj.(*networkingv1beta1.IPAddress), nil
}
