...
type ServiceCIDRClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) networkingv1beta1listers.ServiceCIDRLister
...
func (s *serviceCIDRClusterLister) List(selector labels.Selector) (ret []*networkingv1beta1.ServiceCIDR, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*networkingv1beta1.ServiceCIDR))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get ServiceCIDRs.
func (s *serviceCIDRClusterLister) Cluster(clusterName logicalcluster.Name) networkingv1beta1listers.ServiceCIDRLister {
	return &serviceCIDRLister{indexer: s.indexer, clusterName: clusterName}
}

// serviceCIDRLister implements the networkingv1beta1listers.ServiceCIDRLister interface.
...
func (s *serviceCIDRLister) List(selector labels.Selector) (ret []*networkingv1beta1.ServiceCIDR, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*networkingv1beta1.ServiceCIDR))
	})
	return ret, err
}

// Get retrieves the ServiceCIDR from the indexer for a given workspace and name.
func (s *serviceCIDRLister) Get(name string) (*networkingv1beta1.ServiceCIDR, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(networkingv1beta1.Resource("servicecidrs"), name)
	}
	return obj.(*networkingv1beta1.ServiceCIDR), nil
}
