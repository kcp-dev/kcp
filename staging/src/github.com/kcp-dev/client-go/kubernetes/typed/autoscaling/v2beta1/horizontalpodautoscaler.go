...
type HorizontalPodAutoscalerClusterInterface interface {
	...
	HorizontalPodAutoscalerClusterExpansion
}

type horizontalPodAutoscalersClusterInterface struct {
	clientCache kcpclient.Cache[*typedautoscalingv2beta1.AutoscalingV2beta1Client]
}
...
func (c *horizontalPodAutoscalersClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*autoscalingv2beta1.HorizontalPodAutoscalerList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).HorizontalPodAutoscalers(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all HorizontalPodAutoscalers across all clusters.
func (c *horizontalPodAutoscalersClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).HorizontalPodAutoscalers(v1.NamespaceAll).Watch(ctx, opts)
}

// HorizontalPodAutoscalersNamespacer can scope to objects within a namespace, returning a typedautoscalingv2beta1.HorizontalPodAutoscalerInterface.
type HorizontalPodAutoscalersNamespacer interface {
	Namespace(string) typedautoscalingv2beta1.HorizontalPodAutoscalerInterface
}
...
func (n *horizontalPodAutoscalersNamespacer) Namespace(namespace string) typedautoscalingv2beta1.HorizontalPodAutoscalerInterface {
