...
func (c *replicaSetsClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*appsv1beta2.ReplicaSetList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).ReplicaSets(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all ReplicaSets across all clusters.
func (c *replicaSetsClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).ReplicaSets(v1.NamespaceAll).Watch(ctx, opts)
}

// ReplicaSetsNamespacer can scope to objects within a namespace, returning a typedappsv1beta2.ReplicaSetInterface.
type ReplicaSetsNamespacer interface {
	Namespace(string) typedappsv1beta2.ReplicaSetInterface
}
...
func (n *replicaSetsNamespacer) Namespace(namespace string) typedappsv1beta2.ReplicaSetInterface {
