...
func (c *daemonSetsClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*appsv1beta2.DaemonSetList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).DaemonSets(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all DaemonSets across all clusters.
func (c *daemonSetsClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).DaemonSets(v1.NamespaceAll).Watch(ctx, opts)
}

// DaemonSetsNamespacer can scope to objects within a namespace, returning a typedappsv1beta2.DaemonSetInterface.
type DaemonSetsNamespacer interface {
	Namespace(string) typedappsv1beta2.DaemonSetInterface
}
...
func (n *daemonSetsNamespacer) Namespace(namespace string) typedappsv1beta2.DaemonSetInterface {
