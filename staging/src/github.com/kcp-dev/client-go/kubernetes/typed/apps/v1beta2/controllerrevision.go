...
type controllerRevisionsClusterInterface struct {
	clientCache kcpclient.Cache[*typedappsv1beta2.AppsV1beta2Client]
}
...
func (c *controllerRevisionsClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*appsv1beta2.ControllerRevisionList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).ControllerRevisions(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all ControllerRevisions across all clusters.
func (c *controllerRevisionsClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).ControllerRevisions(v1.NamespaceAll).Watch(ctx, opts)
}

// ControllerRevisionsNamespacer can scope to objects within a namespace, returning a typedappsv1beta2.ControllerRevisionInterface.
type ControllerRevisionsNamespacer interface {
	Namespace(string) typedappsv1beta2.ControllerRevisionInterface
}
...
func (n *controllerRevisionsNamespacer) Namespace(namespace string) typedappsv1beta2.ControllerRevisionInterface {
