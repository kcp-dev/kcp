...
type LeaseClusterInterface interface {
	...
	LeaseClusterExpansion
}

type leasesClusterInterface struct {
	clientCache kcpclient.Cache[*typedcoordinationv1beta1.CoordinationV1beta1Client]
}
...
func (c *leasesClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*coordinationv1beta1.LeaseList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).Leases(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all Leases across all clusters.
func (c *leasesClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).Leases(v1.NamespaceAll).Watch(ctx, opts)
}

// LeasesNamespacer can scope to objects within a namespace, returning a typedcoordinationv1beta1.LeaseInterface.
type LeasesNamespacer interface {
	Namespace(string) typedcoordinationv1beta1.LeaseInterface
}
...
func (n *leasesNamespacer) Namespace(namespace string) typedcoordinationv1beta1.LeaseInterface {
