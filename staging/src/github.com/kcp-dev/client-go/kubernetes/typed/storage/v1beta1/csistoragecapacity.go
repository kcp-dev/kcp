...
type cSIStorageCapacitiesClusterInterface struct {
	clientCache kcpclient.Cache[*typedstoragev1beta1.StorageV1beta1Client]
}
...
func (c *cSIStorageCapacitiesClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*storagev1beta1.CSIStorageCapacityList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).CSIStorageCapacities(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all CSIStorageCapacities across all clusters.
func (c *cSIStorageCapacitiesClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).CSIStorageCapacities(v1.NamespaceAll).Watch(ctx, opts)
}

// CSIStorageCapacitiesNamespacer can scope to objects within a namespace, returning a typedstoragev1beta1.CSIStorageCapacityInterface.
type CSIStorageCapacitiesNamespacer interface {
	Namespace(string) typedstoragev1beta1.CSIStorageCapacityInterface
}
...
func (n *cSIStorageCapacitiesNamespacer) Namespace(namespace string) typedstoragev1beta1.CSIStorageCapacityInterface {
