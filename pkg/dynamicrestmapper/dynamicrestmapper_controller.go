...
var builtinGVKRs []gvkr

func init() {
	builtinGVKRs = make([]gvkr, len(builtinschemas.BuiltInAPIs))
	for i := range builtinschemas.BuiltInAPIs {
		builtinGVKRs[i] = newGVKR(
...
func (c *Controller) enqueueCRD(oldCRD *apiextensionsv1.CustomResourceDefinition, newCRD *apiextensionsv1.CustomResourceDefinition) {
	key, err := kcpcache.DeletionHandlingMetaClusterNamespaceKeyFunc(newCRD)
	if err != nil {
		utilruntime.HandleError(err)
		return
	}

	gatherGVKRs := func(crd *apiextensionsv1.CustomResourceDefinition) []gvkr {
		if crd == nil {
			return nil
		}
		var gvkrs []gvkr
		for _, crdVersion := range crd.Spec.Versions {
			gvkrs = append(gvkrs, newGVKR(
				crd.Spec.Group,
				crdVersion.Name,
				crd.Spec.Names.Kind,
