...
func (o *Options) AddFlags(fss *cliflag.NamedFlagSets) {
	...
	fs.StringSliceVar(&o.ShardsPeerKubeconfigs, "shard-peer-kubeconfig", o.ShardsPeerKubeconfigs, "Repeatable path to a kubeconfig whose named clusters are peer shards; all files' clusters are merged into one peer list. When set, the proxy discovers Shards through the Admin workspace (/services/admin) of the peers - requests are distributed round-robin with failover - instead of watching the root shard. If unset, the --root-kubeconfig server acts as the only seed peer. New shards are discovered without updating the peer list; add stable shards as peers for HA and remove decommissioned shards.")
