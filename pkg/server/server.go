...
func (s *Server) Run(ctx context.Context) error {
	...
	if s.options.Virtual.Enabled {
		if err := s.installVirtualWorkspaces(ctx, genericConfig.LoopbackClientConfig, server, kubeClusterClient, dynamicClusterClient, kcpClusterClient, genericConfig.Authentication, genericConfig.ExternalAddress, preHandlerChainMux); err != nil {
			return err
		}
