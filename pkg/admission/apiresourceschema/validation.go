...
func ValidateAPIResourceSchemaName(name string, spec *apisv1alpha1.APIResourceSchemaSpec, path *field.Path) field.ErrorList {
	...
	if spec.Names.Plural != "" {
		...
		expectedNamePostfix := fmt.Sprintf(".%s.%s", spec.Names.Plural, spec.Group)
		...
		if !namePrefixRE.MatchString(rest) {
			return field.ErrorList{field.Invalid(path, name, "must match patch ^[a-z]([-a-z0-9]*[a-z0-9])? in front of .<resource>.<group>")}
		}
	}
...
func ValidateAPIResourceSchemaSpec(spec *apisv1alpha1.APIResourceSchemaSpec, fldPath *field.Path) field.ErrorList {
	...
	if errs := utilvalidation.IsDNS1123Subdomain(spec.Group); len(errs) > 0 {
