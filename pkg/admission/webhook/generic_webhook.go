...
func (p *WebhookDispatcher) getAPIExportCluster(attr admission.Attributes, clusterName logicalcluster.Name) (logicalcluster.Name, error) {
	...
	for _, apiBinding := range objs {
		...
		// We want to allow installing webhooks on workspaces that are bound to the default compute,
		// without requiring the webhook to be installed in the root compute workspace.
		// TODO(davidfestal): Obviously there shoud be more thought about how to skip some APIExport whose binding is
		// not meaningful. Maybe using giving priority to those havng accepted permission claims on the given resource ?
