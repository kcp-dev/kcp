...
var (
	...
	_ initializers.WantsDynamicClusterClient = (*apiConversionAdmission)(nil)
)
...
func (o *apiConversionAdmission) Validate(ctx context.Context, a admission.Attributes, _ admission.ObjectInterfaces) error {
	if !utilfeature.DefaultFeatureGate.Enabled(features.APIConversion) {
		return nil
	}
	...
	if err != nil {
		...
		// to use pure local CRDs with the CEL engine. This fallback inspects a same-named local CRD.
		gvr := schema.GroupVersionResource{Group: "apiextensions.k8s.io", Version: "v1", Resource: "customresourcedefinitions"}
		crdU, crdErr := o.dynamicClient.Cluster(cluster.Name.Path()).Resource(gvr).Get(ctx, u.GetName(), metav1.GetOptions{})
		if crdErr != nil {
			if apierrors.IsNotFound(crdErr) {
				return admission.NewForbidden(a, fmt.Errorf("error getting APIResourceSchema or verifying CustomResourceDefinition for %s", u.GetName()))
			}
			return admission.NewForbidden(a, fmt.Errorf("error getting CustomResourceDefinition %s: %w", u.GetName(), crdErr))
		}

		crd := &apiextensionsv1.CustomResourceDefinition{}
		if err := runtime.DefaultUnstructuredConverter.FromUnstructured(crdU.Object, crd); err != nil {
			return admission.NewForbidden(a, fmt.Errorf("error decoding CustomResourceDefinition %s: %w", u.GetName(), err))
		}
	...
	structuralSchemas, err := buildStructuralSchemasFromAPIResourceSchema(apiResourceSchema.Spec.Versions)
	if err != nil {
		return admission.NewForbidden(a, err)
	}
...
func (o *apiConversionAdmission) ValidateInitialization() error {
	...
	if o.dynamicClient == nil {
		return fmt.Errorf("dynamicClient is unset")
	}
...
func (o *apiConversionAdmission) SetDynamicClusterClient(clusterInterface kcpdynamic.ClusterInterface) {
	o.dynamicClient = clusterInterface
}

func buildStructuralSchemasFromAPIResourceSchema(versions []apisv1alpha1.APIResourceVersion) (map[string]*structuralschema.Structural, error) {
	structuralSchemas := map[string]*structuralschema.Structural{}
	for i, v := range versions {
		schema, err := v.GetSchema()
		if err != nil {
			return nil, field.Required(field.NewPath("spec", "versions").Index(i).Child("schema"), "is required")
		}

		structuralSchema, err := buildStructuralSchema(v.Name, schema)
		if err != nil {
			return nil, err
		}
...
func buildStructuralSchemasFromCRD(versions []apiextensionsv1.CustomResourceDefinitionVersion) (map[string]*structuralschema.Structural, error) {
	structuralSchemas := map[string]*structuralschema.Structural{}
	for i, v := range versions {
		if v.Schema == nil || v.Schema.OpenAPIV3Schema == nil {
			return nil, field.Required(field.NewPath("spec", "versions").Index(i).Child("schema", "openAPIV3Schema"), "is required")
		}

		structuralSchema, err := buildStructuralSchema(v.Name, v.Schema.OpenAPIV3Schema)
		if err != nil {
			return nil, err
		}
		structuralSchemas[v.Name] = structuralSchema
...
func buildStructuralSchema(versionName string, schema *apiextensionsv1.JSONSchemaProps) (*structuralschema.Structural, error) {
	...
	if err := apiextensionsv1.Convert_v1_JSONSchemaProps_To_apiextensions_JSONSchemaProps(schema, internalJSONSchemaProps, nil); err != nil {
		return nil, fmt.Errorf("failed converting version %s validation to internal version: %w", versionName, err)
	}

	structuralSchema, err := structuralschema.NewStructural(internalJSONSchemaProps)
	if err != nil {
		return nil, fmt.Errorf("error getting structural schema for version %s: %w", versionName, err)
	}
