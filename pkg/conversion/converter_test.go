...
// Test_convert_celMapTransformation ensures CEL results that are maps/lists are converted to
// unstructured-safe JSON values. evalResult.Value() returns CEL-native containers that panic
// inside unstructured.SetNestedField / runtime.DeepCopyJSONValue.
func Test_convert_celMapTransformation(t *testing.T) {
	t.Parallel()
...
`)
	...
	converted, err := converter.convert(context.Background(), v1, schema.GroupVersion{Group: "test.kcp.io", Version: "v2"})
	require.NoError(t, err)

	name, found, err := unstructured.NestedMap(converted.Object, "spec", "name")
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, map[string]interface{}{"first": "alice"}, name)
}
