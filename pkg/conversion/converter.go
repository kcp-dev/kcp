...
func evaluateRule(ctx context.Context, original *unstructured.Unstructured, rule *CompiledRule) (interface{}, error) {
	...
	return celValueToUnstructured(evalResult)
}

func celValueToUnstructured(val ref.Val) (interface{}, error) {
	if types.IsError(val) {
		return nil, fmt.Errorf("CEL evaluation error: %v", val)
	}
	if types.IsUnknown(val) {
		return nil, fmt.Errorf("CEL evaluation returned unknown")
	}
	if val == types.NullValue {
		return nil, nil
	}

	jsonVal, err := val.ConvertToNative(types.JSONValueType)
	if err != nil {
		return nil, fmt.Errorf("error converting CEL result to JSON: %w", err)
	}
	...
	case structpb.NullValue:
		return nil, nil
	default:
		return nil, fmt.Errorf("unexpected CEL JSON value type %T", jsonVal)
	}
}
