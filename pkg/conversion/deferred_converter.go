...
type deferredConverter struct {
	...
	// lock guards delegate and apiConversionResourceVersion
	lock sync.Mutex
	// delegate is the actual converter
	delegate conversion.CRConverter
	// apiConversionResourceVersion is the resource version of the APIConversion used to create delegate
	apiConversionResourceVersion string
...
func (f *deferredConverter) getConverter() (conversion.CRConverter, error) {
	...
	if errors.IsNotFound(err) {
		if f.delegate != nil && f.apiConversionResourceVersion == "" {
			return f.delegate, nil
		}

		if f.crd.Spec.Conversion != nil {
		...
		f.delegate = conversion.NewNOPConverter()
		f.apiConversionResourceVersion = ""
		return f.delegate, nil
	}

	if f.delegate != nil && f.apiConversionResourceVersion == apiConversion.ResourceVersion {
		return f.delegate, nil
	}
	...
	f.apiConversionResourceVersion = apiConversion.ResourceVersion
