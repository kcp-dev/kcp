...
//   - typed scheme conversion for apis.kcp.io
//   - CEL-based conversion rules from APIConversion resources
//   - the "none" conversion strategy
...
// NewCRConverterFactory returns a CRConverterFactory that supports scheme conversion for apis.kcp.io,
// APIConversion-based CEL conversions, and the "none" conversion strategy.
...
// NewConverter returns the appropriate conversion.Converter based on the CRD.
// apis.kcp.io always uses typed scheme conversion (independent of the APIConversion feature gate).
// For other groups, if the CRD identifies as for a "wildcard partial metadata request", the nop
// converter is used. Otherwise, it returns a CEL-based converter if there is an associated
// APIConversion, a nop converter if the strategy is "none", or an error otherwise.
func (f *CRConverterFactory) NewConverter(crd *apiextensionsv1.CustomResourceDefinition) (conversion.CRConverter, error) {
	// Native kcp APIs must keep working even when the APIConversion feature gate is off.
	if crd.Spec.Group == apis.GroupName {
		return &schemaBasedConverter{scheme: f.scheme}, nil
	}
