...
func (a *MaximalPermissionPolicyAuthorizer) Authorize(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			// if attr.GetVerb() == "deletecollection" {
			// 	return DelegateAuthorization(fmt.Sprintf("API Export %q|%q not found, allowing deletecollection", path, relevantBinding.Spec.Reference.Export.Name), a.delegate).Authorize(ctx, attr)
			// }
			// return authorizer.DecisionNoOpinion, fmt.Sprintf("APIExport %q:%q not found", path, relevantBinding.Spec.Reference.Export.Name), nil
		}
