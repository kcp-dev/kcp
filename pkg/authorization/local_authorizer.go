		...
		newAuthorizer: func(clusterName logicalcluster.Name) *rbac.RBACAuthorizer {
			return rbac.New(
				&rbac.RoleGetter{Lister: rbacwrapper.NewMergedRoleLister(
					versionedInformers.Rbac().V1().Roles().Lister().Cluster(clusterName),
					versionedInformers.Rbac().V1().Roles().Lister().Cluster(controlplaneapiserver.LocalAdminCluster),
				)},
				&rbac.RoleBindingLister{Lister: versionedInformers.Rbac().V1().RoleBindings().Lister().Cluster(clusterName)},
				&rbac.ClusterRoleGetter{Lister: rbacwrapper.NewMergedClusterRoleLister(
					versionedInformers.Rbac().V1().ClusterRoles().Lister().Cluster(clusterName),
					versionedInformers.Rbac().V1().ClusterRoles().Lister().Cluster(controlplaneapiserver.LocalAdminCluster),
				)},
				&rbac.ClusterRoleBindingLister{Lister: versionedInformers.Rbac().V1().ClusterRoleBindings().Lister().Cluster(clusterName)},
			)
		},
	}
