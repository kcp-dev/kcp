...
type DelegatedAuthorizerFactory func(clusterName logicalcluster.Name, clusterClient kubeclient.Interface) (authorizer.Authorizer, error)
...
func NewDelegatedAuthorizer(clusterName logicalcluster.Name, clusterClient kubeclient.Interface) (authorizer.Authorizer, error) {
	delegatingAuthorizerConfig := &authorizerfactory.DelegatingAuthorizerConfig{
		SubjectAccessReviewClient: &clusterAwareAuthorizationV1Client{
			AuthorizationV1Interface: clusterClient.AuthorizationV1(),
			cluster:                  clusterName,
		},
