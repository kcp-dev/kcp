...
func TestWithWarrants(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			user:       &user.DefaultInfo{Name: "unknown", Extra: map[string][]string{validation.WarrantExtraKey: {`{"user":"unknown", "extra": {"authorization.kcp.io/warrants": ["{\"user\":\"user-access\"}"]}}`}}},
			wantDec:    authorizer.DecisionAllow,
			wantReason: "user-unknown; warrants: [user-unknown; warrants: [user-access]]",
		},
		{
			name:       "nested warrants with deny in the middle",
			user:       &user.DefaultInfo{Name: "unknown", Extra: map[string][]string{validation.WarrantExtraKey: {`{"user":"user-deny", "extra": {"authorization.kcp.io/warrants": ["{\"user\":\"user-access\"}"]}}`}}},
			wantDec:    authorizer.DecisionNoOpinion,
			wantReason: "user-unknown; warrants: [user-deny]",
		},
		{
