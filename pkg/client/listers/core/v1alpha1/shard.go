...
type ShardClusterLister interface {
	...
	List(selector labels.Selector) (ret []*corev1alpha1.Shard, err error)
...
func (s *shardClusterLister) List(selector labels.Selector) (ret []*corev1alpha1.Shard, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*corev1alpha1.Shard))
	})
	return ret, err
}
...
type ShardLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*corev1alpha1.Shard, error)
...
func (s *shardLister) List(selector labels.Selector) (ret []*corev1alpha1.Shard, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*corev1alpha1.Shard))
	})
	return ret, err
}

// Get retrieves the Shard from the indexer for a given workspace and name.
func (s *shardLister) Get(name string) (*corev1alpha1.Shard, error) {
...
// List lists all Shards in the indexer for a workspace.
func (s *shardScopedLister) List(selector labels.Selector) (ret []*corev1alpha1.Shard, err error) {
...
// Get retrieves the Shard from the indexer for a given workspace and name.
func (s *shardScopedLister) Get(name string) (*corev1alpha1.Shard, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
