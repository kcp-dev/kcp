...
type TransformResourceBeforeFunc func(ctx context.Context, resource *unstructured.Unstructured, subresources ...string) (transformed *unstructured.Unstructured, err error)
type TransformResourceAfterFunc func(ctx context.Context, resource *unstructured.Unstructured, eventType *watch.EventType, subresources ...string) (transformed *unstructured.Unstructured, err error)
...
func TransformsResource(transformerName string, before TransformResourceBeforeFunc, after TransformResourceAfterFunc) Transformer {
	...
	if before != nil {
		loggedBefore = func(action string) TransformResourceBeforeFunc {
			return func(ctx context.Context, resource *unstructured.Unstructured, subresources ...string) (transformed *unstructured.Unstructured, err error) {
				name := "nil"
				if resource != nil {
					name = resource.GetName()
				}
				klog.Infof("Before%s(%s-ResourceTransformation) %s - %v", action, t.Name, name, subresources)
				transformed, err = before(ctx, resource, subresources...)
				if err != nil {
	...
	if after != nil {
		loggedAfter = func(action string) TransformResourceAfterFunc {
			return func(ctx context.Context, resource *unstructured.Unstructured, eventType *watch.EventType, subresources ...string) (transformed *unstructured.Unstructured, err error) {
				name := "nil"
				if resource != nil {
					name = resource.GetName()
				}
				...
				transformed, err = after(ctx, resource, eventType, subresources...)
				if err != nil {
	...
	if before != nil {
		t.BeforeCreate = func(ctx context.Context, obj *unstructured.Unstructured, options metav1.CreateOptions, subresources ...string) (context.Context, *unstructured.Unstructured, metav1.CreateOptions, []string, error) {
			if transformedResource, err := loggedBefore("Create")(ctx, obj, subresources...); err != nil {
		...
		t.BeforeUpdate = func(ctx context.Context, obj *unstructured.Unstructured, options metav1.UpdateOptions, subresources ...string) (context.Context, *unstructured.Unstructured, metav1.UpdateOptions, []string, error) {
			if transformedResource, err := loggedBefore("Update")(ctx, obj, subresources...); err != nil {
				return ctx, obj, options, subresources, err
			} else {
				return ctx, transformedResource, options, subresources, nil
			}
	...
	if after != nil {
		t.AfterCreate = func(ctx context.Context, obj *unstructured.Unstructured, options metav1.CreateOptions, subresources []string, result *unstructured.Unstructured) (*unstructured.Unstructured, error) {
			if transformedResource, err := loggedAfter("Create")(ctx, obj, nil, subresources...); err != nil {
				return obj, err
			} else {
		...
		t.AfterUpdate = func(ctx context.Context, obj *unstructured.Unstructured, options metav1.UpdateOptions, subresources []string, result *unstructured.Unstructured) (*unstructured.Unstructured, error) {
			if transformedResource, err := loggedAfter("Update")(ctx, obj, nil, subresources...); err != nil {
				return obj, err
			} else {
				return transformedResource, nil
			}
		...
		t.AfterGet = func(ctx context.Context, name string, options metav1.GetOptions, subresources []string, result *unstructured.Unstructured) (*unstructured.Unstructured, error) {
			if transformedResource, err := loggedAfter("Get")(ctx, result, nil, subresources...); err != nil {
				return nil, err
			} else {
				return transformedResource, nil
			}
		...
		t.AfterList = func(ctx context.Context, opts metav1.ListOptions, result *unstructured.UnstructuredList) (*unstructured.UnstructuredList, error) {
			...
			for _, item := range result.Items {
				item := item
				if transformed, err := afterList(ctx, &item, nil); err != nil {
		...
		t.AfterWatch = func(ctx context.Context, opts metav1.ListOptions, result watch.Interface) (watch.Interface, error) {
			...
			transformingWatcher := NewTransformingWatcher(result, func(event watch.Event) (transformed watch.Event, skipped bool) {
				...
				case watch.Added, watch.Modified, watch.Deleted:
					...
					} else if transformedResource, err := afterWatchEvent(ctx, resource, &eventType); err != nil {
						if errors.IsNotFound(err) {
							skipped = true
						} else {
							transformed.Type = watch.Error
