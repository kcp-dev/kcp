...
func NewClusterWorkspaceCache(workspaces cache.SharedIndexInformer, tenancyClientGetter func(orgName string) tenancyv1alpha1.TenancyV1alpha1Interface, defaultNodeSelector string) *ClusterWorkspaceCache {
	...
	return &ClusterWorkspaceCache{
		ClientGetter:        tenancyClientGetter,
...
type ClusterWorkspaceCache struct {
	ClientGetter        func(orgName string) tenancyv1alpha1.TenancyV1alpha1Interface
	Store               cache.Indexer
...
func (c *ClusterWorkspaceCache) GetWorkspace(lclusterName, workspaceName string) (*workspaceapi.ClusterWorkspace, error) {
	...
	if exists {
		...
		clusterWorkspace, err = c.ClientGetter(workspaceName).ClusterWorkspaces().Get(context.TODO(), workspaceName, metav1.GetOptions{})
