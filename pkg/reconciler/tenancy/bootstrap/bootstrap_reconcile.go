...
func (c *controller) reconcile(ctx context.Context, workspace *tenancyv1alpha1.ClusterWorkspace) error {
	...
	initializer := tenancyv1alpha1.ClusterWorkspaceInitializer{
		...
		Path: c.initializerPath,
	}
	if !initialization.InitializerPresent(initializer, workspace.Status.Initializers) {
		return nil
	}
	...
	workspace.Status.Initializers = initialization.EnsureInitializerAbsent(initializer, workspace.Status.Initializers)
