...
func TestFindSelectorInWorkspace(t *testing.T) {
	...
	tests := map[string]struct {
		...
		workspaceCluster  *corev1alpha1.LogicalCluster
		...
		"returns nil when workspace LogicalCluster does not exist": {
			workspacePath: logicalcluster.NewPath("root:parent"),
			exportRef: tenancyv1alpha1.APIExportReference{
				Path:   "root:export-ws",
				Export: "test.export",
			...
			getClusterError: apierrors.NewNotFound(corev1alpha1.Resource("logicalclusters"), "parent"),
			expectedFound:   false,
		},
	}
			...
			c := &DefaultAPIBindingController{
				getLogicalClusterByPath: func(path logicalcluster.Path) (*corev1alpha1.LogicalCluster, error) {
					if tc.getClusterError != nil {
						return nil, tc.getClusterError
					}
					if tc.workspaceCluster != nil && path == tc.workspacePath {
						return tc.workspaceCluster, nil
					}
