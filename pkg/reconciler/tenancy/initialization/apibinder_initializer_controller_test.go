...
// TestProcessConditionIsolation verifies that process() only writes the conditions this
...
func TestProcessConditionIsolation(t *testing.T) {
	...
	// The "live" object on the API server already has a condition written by a different
	// controller (WorkspaceAPIBindingsReconciled). After our controller runs, that foreign
	...
	// Simulate the cached object seen by the controller before reconcile — no conditions yet.
	...
	// (reconcile sets it to False/WorkspaceTypeInvalid because getWorkspaceType returns NotFound)
