...
func (c *controller) processUpstreamView(ctx context.Context, key string) error {
	...
	deployment, err := c.deploymentLister.Cluster(lclusterName).Deployments(namespace).Get(name)
	if err != nil {
		return err
	}
	...
	if !equality.Semantic.DeepEqual(deployment.Annotations, updated.Annotations) {
		...
		_, err = c.kubeClusterClient.Cluster(lclusterName).AppsV1().Deployments(namespace).Update(ctx, updated, metav1.UpdateOptions{})
	}
...
func (c *controller) processSyncerView(ctx context.Context, key string) error {
	...
	for _, condition := range consolidatedConditions {
		summarizedStatus.Conditions = append(summarizedStatus.Conditions, condition)
	}
	...
	if !equality.Semantic.DeepEqual(deployment.Status, summarized.Status) {
		...
		_, err = c.kubeClusterClient.Cluster(lclusterName).AppsV1().Deployments(namespace).UpdateStatus(ctx, summarized, metav1.UpdateOptions{})
	}
	return err
}
