...
func (s *scopedGenericNamespaceLister) List(selector labels.Selector) (ret []runtime.Object, err error) {
	// QUOTA HACK: to support quota for cluster-scoped resources, we've hacked the k8s quota to use namespace="" when
	// checking quota for cluster-scoped resources. But because all of the upstream quota code is written to only
