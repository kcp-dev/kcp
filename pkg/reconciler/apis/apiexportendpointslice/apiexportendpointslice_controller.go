...
func (c *controller) enqueueAPIExportEndpointSlicesForAPIExport(export *apisv1alpha1.APIExport, logger logr.Logger, logSuffix string) {
...
func (c *controller) enqueueAllAPIExportEndpointSlices(shard *corev1alpha1.Shard, logger logr.Logger, logSuffix string) {
	...
	logger = logger.WithValues("shard", shard.Name)
	for i := range list {
		...
		logging.WithQueueKey(logger, key).V(4).Info("queuing all APIExportEndpointSlice because Shard changed")
		c.queue.Add(key)
	}
}
