...
func TestIndexAPIBindingByWorkspaceExport(t *testing.T) {
	tests := map[string]struct {
		...
		"has a workspace reference": {
			...
			want:    []string{kcpcache.ToClusterAwareKey("root:workspace1", "", "export1")},
			wantErr: false,
		},
	}
...
func TestIndexAPIExportByAPIResourceSchemas(t *testing.T) {
	tests := map[string]struct {
		...
		"valid APIExport": {
			...
			want: []string{
				kcpcache.ToClusterAwareKey("root:default", "", "schema1"),
				kcpcache.ToClusterAwareKey("root:default", "", "some-other-schema"),
			},
			wantErr: false,
		},
	}
