...
const (
	...
	// DefaultAPIExportName is the default name of the APIExport for a location domain. In the future, we might
	// offer a mechanism to have multiple APIExports for a location domain, e.g. to export different sets of APIs.
	DefaultAPIExportName = "workloads"

	indexAPIResourcesSchemaByWorkspace     = "APIResourcesSchemaByWorkspace"
	indexNegotiatedAPIResourcesByWorkspace = "negotiatedAPIResourcesByWorkspace"
)
...
func NewController(
	...
	if err := c.apiResourceSchemaIndexer.AddIndexers(cache.Indexers{
		indexAPIResourcesSchemaByWorkspace: indexAPIResourceSchemaByWorkspace,
	}); err != nil {
	...
	if err := negotiatedAPIResourceInformer.Informer().AddIndexers(cache.Indexers{
		indexNegotiatedAPIResourcesByWorkspace: indexNegotiatedAPIResourceByWorkspace,
	}); err != nil {
		return nil, err
	}
...
func (c *controller) enqueueNegotiatedAPIResource(obj interface{}) {
	...
	key := clusters.ToClusterAwareKey(clusterName, DefaultAPIExportName)

	if _, err := c.apiExportsLister.Get(key); err == nil {
		// if there is no APIExport yet, just skip this.
