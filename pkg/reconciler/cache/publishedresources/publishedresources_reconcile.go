...
func (c *Controller) reconcile(ctx context.Context, cluster logicalcluster.Name, publishedResource *cachev1alpha1.PublishedResource) (bool, error) {
	reconcilers := []reconciler{
		...
		&purge{
			deleteSelectedCacheResources: func(ctx context.Context, publishedResource *cachev1alpha1.PublishedResource) error {
				return c.deleteSelectedCacheResources(ctx, cluster, publishedResource)
			},
		},
		&counter{
			listSelectedLocalResources: func(ctx context.Context, publishedResource *cachev1alpha1.PublishedResource) (*unstructured.UnstructuredList, error) {
				return c.listSelectedLocalResources(ctx, cluster, publishedResource)
			},
			listSelectedCacheResources: func(ctx context.Context, publishedResource *cachev1alpha1.PublishedResource) (*cachev1alpha1.PublishedObjectList, error) {
				return c.listSelectedCacheResources(ctx, cluster, publishedResource)
			},
		},
		&replication{
	...
	for _, r := range reconcilers {
		...
		status, err = r.reconcile(ctx, publishedResource)
		if err != nil {
...
func (c *Controller) listSelectedLocalResources(ctx context.Context, cluster logicalcluster.Name, publishedResource *cachev1alpha1.PublishedResource) (*unstructured.UnstructuredList, error) {
...
func (c *Controller) deleteSelectedCacheResources(ctx context.Context, cluster logicalcluster.Name, publishedResource *cachev1alpha1.PublishedResource) error {
	gvr := schema.GroupVersionResource{
		Group:    publishedResource.Spec.Group,
		Version:  publishedResource.Spec.Version,
		Resource: publishedResource.Spec.Resource,
	...
	if len(publishedResource.Spec.LabelSelector.MatchLabels) > 0 {
		l := labels.SelectorFromSet(publishedResource.Spec.LabelSelector.MatchLabels)
		r, _ := selector.Requirements()
		selector = l.Add(r...)
	}

	ctx = cacheclient.WithShardInContext(ctx, shard.New(c.shardName))
	return c.kcpCacheClient.Cluster(cluster.Path()).CacheV1alpha1().PublishedObjects().DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
		LabelSelector: selector.String(),
	})
}

func (c *Controller) listSelectedCacheResources(ctx context.Context, cluster logicalcluster.Name, publishedResource *cachev1alpha1.PublishedResource) (*cachev1alpha1.PublishedObjectList, error) {
	gvr := schema.GroupVersionResource{
		Group:    publishedResource.Spec.Group,
		Version:  publishedResource.Spec.Version,
		Resource: publishedResource.Spec.Resource,
	...
	ctx = cacheclient.WithShardInContext(ctx, shard.New(c.shardName))
	resources, err := c.kcpCacheClient.Cluster(cluster.Path()).CacheV1alpha1().PublishedObjects().List(ctx, metav1.ListOptions{
		LabelSelector: selector.String(),
	})
	if err != nil {
		return nil, err
...
func (c *Controller) ensureSecretNamespaceExists(ctx context.Context, clusterName logicalcluster.Name) {
	...
	if _, err := c.getNamespace(clusterName, c.secretNamespace); errors.IsNotFound(err) {
		...
		if err := c.createNamespace(ctx, clusterName.Path(), ns); err != nil && !errors.IsAlreadyExists(err) {
			logger.Error(err, "error creating namespace for PublishedResource secret identities")
...
func (c *Controller) updateOrVerifyIdentitySecretHash(ctx context.Context, clusterName logicalcluster.Name, publishedResource *cachev1alpha1.PublishedResource) error {
	secret, err := c.getSecret(ctx, clusterName, c.secretNamespace, publishedResource.Name)
	if err != nil {
		return err
	}
	...
	if publishedResource.Status.IdentityHash == "" {
		publishedResource.Status.IdentityHash = hash
	}

	if publishedResource.Status.IdentityHash != hash {
		return fmt.Errorf("hash mismatch: identity secret hash %q must match status.identityHash %q", hash, publishedResource.Status.IdentityHash)
	}

	conditions.MarkTrue(publishedResource, cachev1alpha1.PublishedResourceIdentityValid)
