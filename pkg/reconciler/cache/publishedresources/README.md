# PublishedResources controller

This controller acts on `PublishedResources` objects and based on configured GroupVersionKind - replicated them into cache server.
To replicate objects 'wrapper' object is used `PublishedObject`. Objects are named based on the original gvr. Example:

Object targeted by `PublishedResource` bellow:
```yaml
...
Main controller (`publishedresources`) for each `PublishedResource` starts child replication controller with 2 informers:
* Local - targeting native type (in example `instances`) to get every change for it.
* Global - targeting `PublishedObject` from the cache server

It will observe both cache and local states and makes sure object are replicated.
On deletion of `PublishedResources` cache is purged and controller stopped after purge is done.


## Delete flow:

1. PublishedResource is deleted
