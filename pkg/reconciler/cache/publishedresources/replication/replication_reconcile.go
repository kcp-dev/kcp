...
func (c *Controller) reconcile(ctx context.Context, gvrKey string) error {
	...
	r := &replicationReconciler{
		...
		createObject: func(ctx context.Context, cluster logicalcluster.Name, obj *unstructured.Unstructured) (*cachev1alpha1.PublishedObject, error) {
			...
			cacheObj := &cachev1alpha1.PublishedObject{
				TypeMeta: metav1.TypeMeta{
					Kind:       cache.PublishedObjectKind,
				...
				Spec: cachev1alpha1.PublishedObjectSpec{
			...
			u, err := c.kcpCacheClient.Cluster(cluster.Path()).CacheV1alpha1().PublishedObjects().Create(ctx, cacheObj, metav1.CreateOptions{})
			return u, err
		},
		updateObject: func(ctx context.Context, cluster logicalcluster.Name, obj *unstructured.Unstructured) (*cachev1alpha1.PublishedObject, error) {
			gvr := obj.GroupVersionKind()
			...
			return c.kcpCacheClient.Cluster(cluster.Path()).CacheV1alpha1().PublishedObjects().Update(ctx, cacheObj, metav1.UpdateOptions{})
		},
		deleteObject: func(ctx context.Context, cluster logicalcluster.Name, ns, name string, gvr schema.GroupVersionResource) error {
			...
			return c.kcpCacheClient.Cluster(cluster.Path()).CacheV1alpha1().PublishedObjects().Delete(ctx, nameCache, metav1.DeleteOptions{})
		},
	}
	defer c.callback()
