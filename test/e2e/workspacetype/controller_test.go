		...
		},
		{
			...
			work: func(ctx context.Context, t *testing.T, server runningServer) {
				...
				err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
					thisWorkspace, err := server.kcpClusterClient.TenancyV1alpha1().ThisWorkspaces().Cluster(tenancy.Cluster(workspace.Status.Cluster).LogicalCluster()).Get(ctx, tenancyv1alpha1.ThisWorkspaceName, metav1.GetOptions{})
					require.NoError(t, err)
					...
					_, err = server.kcpClusterClient.TenancyV1alpha1().ThisWorkspaces().Cluster(tenancy.Cluster(workspace.Status.Cluster).LogicalCluster()).UpdateStatus(ctx, thisWorkspace, metav1.UpdateOptions{})
					return err
				})
				require.NoError(t, err)
