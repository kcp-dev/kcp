	...
	var testCases = []struct {
	...
	}{
		{
			...
			work: func(ctx context.Context, t *testing.T, server runningServer) {
				...
				workspace, err := server.kcpClusterClient.TenancyV1beta1().Workspaces().Create(logicalcluster.WithCluster(ctx, server.orgClusterName), &tenancyv1beta1.Workspace{ObjectMeta: metav1.ObjectMeta{Name: "myapp"}}, metav1.CreateOptions{})
				require.NoError(t, err, "failed to create workspace")
				server.Artifact(t, func() (runtime.Object, error) {
					return server.kcpClusterClient.TenancyV1beta1().Workspaces().Get(logicalcluster.WithCluster(ctx, server.orgClusterName), workspace.Name, metav1.GetOptions{})
				})
				server.Artifact(t, func() (runtime.Object, error) {
					return server.kcpClusterClient.TenancyV1beta1().Workspaces().Get(logicalcluster.WithCluster(ctx, server.orgClusterName), workspace.Name, metav1.GetOptions{})
				})
				...
				require.Eventually(t, func() bool {
					workspace, err = server.kcpClusterClient.TenancyV1beta1().Workspaces().Get(logicalcluster.WithCluster(ctx, server.orgClusterName), workspace.Name, metav1.GetOptions{})
					if err != nil {
				...
				require.Equalf(t, workspace.Spec.Type, tenancyv1alpha1.ResolvedWorkspaceTypeReference{
		...
		},
		{
			...
			work: func(ctx context.Context, t *testing.T, server runningServer) {
				...
				workspace, err := server.kcpClusterClient.TenancyV1beta1().Workspaces().Create(logicalcluster.WithCluster(ctx, universal), &tenancyv1beta1.Workspace{
				...
				require.Eventually(t, func() bool {
					...
					workspace, err = server.kcpClusterClient.TenancyV1beta1().Workspaces().Create(logicalcluster.WithCluster(ctx, universal), &tenancyv1beta1.Workspace{
						ObjectMeta: metav1.ObjectMeta{Name: "myapp"},
						Spec:       tenancyv1beta1.WorkspaceSpec{Type: referenceFor(cwt)},
					}, metav1.CreateOptions{})
					if err != nil {
				...
				server.Artifact(t, func() (runtime.Object, error) {
					return server.kcpClusterClient.TenancyV1beta1().Workspaces().Get(logicalcluster.WithCluster(ctx, universal), "myapp", metav1.GetOptions{})
				})
				require.Equal(t, workspace.Spec.Type, tenancyv1alpha1.ResolvedWorkspaceTypeReference{
					ClusterWorkspaceTypeReference: tenancyv1alpha1.ClusterWorkspaceTypeReference{
						Name: "foo",
						Path: logicalcluster.From(cwt).String(),
					},
					Cluster: tenancy.Cluster(logicalcluster.From(cwt).String()),
				})

				t.Logf("Expect workspace to become ready because there are no initializers")
				require.Eventually(t, func() bool {
					workspace, err = server.kcpClusterClient.TenancyV1beta1().Workspaces().Get(logicalcluster.WithCluster(ctx, universal), workspace.Name, metav1.GetOptions{})
					if err != nil {
						t.Logf("error getting workspace: %v", err)
					}
					return err == nil && workspace.Status.Phase == tenancyv1alpha1.WorkspacePhaseReady
		...
		},
		{
			...
			work: func(ctx context.Context, t *testing.T, server runningServer) {
				...
				framework.AdmitExistingWorkspaceAccess(t, ctx, kubeClusterClient, universal, []string{"user-1"}, nil, []string{"access"})
				...
				require.Eventually(t, func() bool {
					// note: admission is informer based and hence would race with this create call
					workspace, err = user1KcpClient.TenancyV1beta1().Workspaces().Create(logicalcluster.WithCluster(ctx, universal), &tenancyv1beta1.Workspace{
						ObjectMeta: metav1.ObjectMeta{Name: "myapp"},
						Spec:       tenancyv1beta1.WorkspaceSpec{Type: referenceFor(cwt)},
					}, metav1.CreateOptions{})
					if err != nil {
		...
		},
		{
			...
			work: func(ctx context.Context, t *testing.T, server runningServer) {
				...
				err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
					thisWorkspace, err := server.kcpClusterClient.TenancyV1alpha1().ThisWorkspaces().Get(logicalcluster.WithCluster(ctx, tenancy.Cluster(workspace.Status.Cluster).LogicalCluster()), tenancyv1alpha1.ThisWorkspaceName, metav1.GetOptions{})
					require.NoError(t, err)
					...
					_, err = server.kcpClusterClient.TenancyV1alpha1().ThisWorkspaces().UpdateStatus(logicalcluster.WithCluster(ctx, tenancy.Cluster(workspace.Status.Cluster).LogicalCluster()), thisWorkspace, metav1.UpdateOptions{})
					return err
				})
				require.NoError(t, err)
