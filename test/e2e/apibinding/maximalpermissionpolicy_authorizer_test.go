...
func TestMaximalPermissionPolicyAuthorizer(t *testing.T) {
	...
	for serviceProvider, consumer := range m {
		...
		if serviceProvider == rbacServiceProviderPath {
			...
			// still create cowboy2 for the admin to deleted after the
			// APIExport deletion
			_, err = cowboyclient.Create(ctx, cowboy2, metav1.CreateOptions{})
			require.NoError(t, err)

			// Create another cowboy that will be deleted unpon the
			// deletion of the APIBinding.
			...
			t.Logf("user-2 can list cowboys before deleting APIExport in consumer workspace %q", consumer)
			...
			t.Logf("user-2 can list cowboys despite deleted APIExport in consumer workspace %q", consumer)
			_, err = user2Client.Cluster(consumer).WildwestV1alpha1().Cowboys("default").List(ctx, metav1.ListOptions{})
			require.NoError(t, err)
			...
			t.Logf("admin should be able to list the cowboys in consumer workspace %q", consumer)
			...
			t.Logf("admin should be able to delete the existing cowboy in consumer workspace %q", consumer)
			...
			t.Logf("APIBinding deletion should not error")
			...
			t.Logf("The CRDs should be deleted in consumer workspace %q", consumer)
