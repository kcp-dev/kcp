...
func TestAPIBinding(t *testing.T) {
	...
	t.Logf("Install apiresourceschemas, apiexports CRDs into workspace %q", sourceWorkspace.Name)
	sourceCRDClient := apiExtensionsClients.Cluster(sourceWorkspaceClusterName).ApiextensionsV1().CustomResourceDefinitions()
	err = crds.Create(ctx, sourceCRDClient,
	...
	t.Logf("Install apibindings CRDs into workspace %q", targetWorkspace.Name)
	targetCRDClient := apiExtensionsClients.Cluster(targetWorkspaceClusterName).ApiextensionsV1().CustomResourceDefinitions()
	err = crds.Create(ctx, targetCRDClient,
