...
func TestWorkspaceAuth(t *testing.T) {
	...
	authConfig := authfixtures.CreateWorkspaceOIDCAuthentication(t, ctx, kcpClusterClient, baseWsPath, mockOidc, ca, nil)
	wsOidcType := authfixtures.CreateWorkspaceType(t, ctx, kcpClusterClient, baseWsPath, "with-oidc", authConfig)
	...
	_, err = kubeClusterClient.Cluster(providerPath).RbacV1().ClusterRoles().Create(ctx, providerCR, metav1.CreateOptions{})
	require.NoError(t, err)
	...
	_, err = kubeClusterClient.Cluster(providerPath).RbacV1().ClusterRoleBindings().Create(ctx, providerCRB, metav1.CreateOptions{})
	require.NoError(t, err)
	...
	_, err = kubeClusterClient.Cluster(consumerPath).RbacV1().ClusterRoles().Create(ctx, consumerCR, metav1.CreateOptions{})
	require.NoError(t, err)
	...
	_, err = kubeClusterClient.Cluster(consumerPath).RbacV1().ClusterRoleBindings().Create(ctx, consumerCRB, metav1.CreateOptions{})
	require.NoError(t, err)
	...
	kcptestinghelpers.Eventually(t, func() (bool, string) {
		_, err = oidcKcpClient.Cluster(consumerPath).ApisV1alpha2().APIBindings().Create(ctx, cowboysAPIBinding, metav1.CreateOptions{})
...
func TestServiceAccount(t *testing.T) {
	...
	kcptestinghelpers.Eventually(t, func() (bool, string) {
		_, err = saClient.Cluster(consumerPath).ApisV1alpha2().APIBindings().Create(ctx, cowboysAPIBinding, metav1.CreateOptions{})
		return err == nil, fmt.Sprintf("Error creating APIBinding: %v", err)
	}, wait.ForeverTestTimeout, time.Millisecond*100)
...
//  1. The user is a global KCP user but is restricted by warrants to
...
func TestScopedUser(t *testing.T) {
	...
	testUser := "user-1"
	framework.AdmitWorkspaceAccess(ctx, t, kubeClusterClient, consumerPath, []string{testUser}, nil, true)

	t.Log("Admit user-2 as a normal user to the consumer workspace")
	impersonateUser := "user-2"
	framework.AdmitWorkspaceAccess(ctx, t, kubeClusterClient, consumerPath, []string{impersonateUser}, nil, false)
	...
	userCfg.Impersonate = rest.ImpersonationConfig{
		...
		Extra: map[string][]string{
			rbacregistryvalidation.ScopeExtraKey: []string{"cluster:" + consumerWS.Spec.Cluster},
		},
	}
	...
	kcptestinghelpers.Eventually(t, func() (bool, string) {
		_, err = userClient.Cluster(consumerPath).ApisV1alpha2().APIBindings().Create(ctx, cowboysAPIBinding, metav1.CreateOptions{})
		return err == nil, fmt.Sprintf("Error creating APIBinding: %v", err)
	}, wait.ForeverTestTimeout, time.Millisecond*100)
