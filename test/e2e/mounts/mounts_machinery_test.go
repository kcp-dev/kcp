...
func TestMountsMachinery(t *testing.T) {
	...
	orgSource, _ := framework.NewOrganizationFixture(t, server, framework.WithNameSuffix("source"))
	orgDestination, _ := framework.NewOrganizationFixture(t, server, framework.WithNameSuffix("destination"))
	...
	err = helpers.CreateResourceFromFS(ctx, dynamicClusterClient.Cluster(sourcePath), mapper, nil, "apiresourceschema_kubecluster.yaml", testFiles)
	...
	t.Log("Workspace should have WorkspaceMountReady conditions true")
	framework.Eventually(t, func() (bool, string) {
