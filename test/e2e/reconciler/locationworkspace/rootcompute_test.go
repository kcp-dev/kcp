...
func TestRootComputeWorkspace(t *testing.T) {
	...
	syncerFixture := framework.NewSyncerFixture(t, source, computePath,
	...
	framework.NewBindCompute(t, consumerPath, source,
		framework.WithAPIExportsWorkloadBindOption("root:compute:kubernetes"),
		framework.WithLocationWorkspaceWorkloadBindOption(computePath),
	).Bind(t)

	t.Logf("Wait for being able to list Services in the user workspace")
	require.Eventually(t, func() bool {
		_, err := kubeClusterClient.Cluster(consumerPath).CoreV1().Services("default").List(ctx, metav1.ListOptions{})
	...
	_, err = kubeClusterClient.Cluster(consumerPath).CoreV1().Services("default").Create(ctx, &corev1.Service{
	...
	framework.Eventually(t, func() (bool, string) {
		_, err := kubeClusterClient.Cluster(consumerPath).AppsV1().Deployments("default").List(ctx, metav1.ListOptions{})
		if err != nil {
	...
	_, err = kubeClusterClient.Cluster(consumerPath).AppsV1().Deployments("default").Create(ctx, &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name: "first",
			Labels: map[string]string{
				"test.workload.kcp.io": syncTargetName,
	...
	discoverClient := kubeClusterClient.Cluster(consumerPath).Discovery()
	...
	scaleClient := scale.New(kubeClusterClient.Cluster(consumerPath).AppsV1().RESTClient(), restMapper, dynamic.LegacyAPIPathResolverFunc, scaleKindResolver)
