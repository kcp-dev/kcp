...
func TestAPIExportAuthorizers(t *testing.T) {
	...
	serviceProvider1Workspace := framework.NewWorkspaceFixture(t, server, org.Path(), framework.WithName("provider-a"))
	serviceProvider2Workspace := framework.NewWorkspaceFixture(t, server, org.Path(), framework.WithName("provider-b"))
	tenantWorkspace := framework.NewWorkspaceFixture(t, server, org.Path(), framework.WithName("tenant"))
	tenantShadowCRDWorkspace := framework.NewWorkspaceFixture(t, server, org.Path(), framework.WithName("tenant-shadowed-crd"))
	...
	framework.AdmitWorkspaceAccess(t, ctx, kubeClient, serviceProvider1Workspace.Path(), []string{"user-1"}, nil, true)
	framework.AdmitWorkspaceAccess(t, ctx, kubeClient, serviceProvider2Workspace.Path(), []string{"user-2"}, nil, true)
	framework.AdmitWorkspaceAccess(t, ctx, kubeClient, tenantWorkspace.Path(), []string{"user-3"}, nil, true)
	framework.AdmitWorkspaceAccess(t, ctx, kubeClient, tenantShadowCRDWorkspace.Path(), []string{"user-3"}, nil, true)

	t.Logf("install sherriffs API resource schema, API export, permissions for user-3 to be able to bind to the export in service provider workspace %q", serviceProvider1Workspace)
	require.NoError(t, apply(t, ctx, serviceProvider1Workspace, framework.UserConfig("user-1", rest.CopyConfig(cfg)),
	...
	framework.Eventually(t, func() (done bool, str string) {
		sheriffExport, err := user1KcpClient.Cluster(serviceProvider1Workspace.Path()).ApisV1alpha1().APIExports().Get(ctx, "wild.wild.west", metav1.GetOptions{})
		if err != nil {
	...
	t.Logf("install cowboys API resource schema, API export, and permissions for user-3 to be able to bind to the export in service provider workspace %q", serviceProvider2Workspace)
	require.NoError(t, apply(t, ctx, serviceProvider2Workspace, framework.UserConfig("user-2", rest.CopyConfig(cfg)),
		&apisv1alpha1.APIResourceSchema{
				...
				PermissionClaims: []apisv1alpha1.PermissionClaim{
					{
						...
						ResourceSelector: []apisv1alpha1.ResourceSelector{
							{Name: "claimed-cm", Namespace: "claimed-ns-by-serviceprovider-2"},
							{Name: "claimed-deadlock-cm", Namespace: "deadlock-cm-namespace"},
						},
						Verbs: apisv1alpha1.Verbs{Claimed: []string{"get", "list", "watch"}, RestrictTo: []string{"*"}},
					},
					{
	...
	t.Logf("bind cowboys and claimed sherriffs in the tenant workspace %q", tenantWorkspace)
	require.NoError(t, apply(t, ctx, tenantWorkspace, framework.UserConfig("user-3", rest.CopyConfig(cfg)),
	...
	framework.Eventually(t, func() (success bool, reason string) {
		groups, err := user3tenantWorkspaceKcpClient.Cluster(tenantWorkspace.Path()).Discovery().ServerGroups()
		if err != nil {
			return false, fmt.Sprintf("error retrieving consumer workspace %q group discovery: %v", tenantWorkspace, err)
		}
	...
	t.Logf("Install cowboys CRD and also bind the conflicting cowboys API export in tenant workspace %q", tenantShadowCRDWorkspace)
	require.NoError(t, apply(t, ctx, tenantShadowCRDWorkspace, framework.UserConfig("user-3", rest.CopyConfig(cfg)),
	...
	t.Logf("Waiting for cowboys CRD to be ready in tenant workspace %q", tenantShadowCRDWorkspace)
	...
	framework.Eventually(t, func() (bool, string) {
		cowboysCRD, err := user3APIExtensionsClient.Cluster(tenantShadowCRDWorkspace.Path()).ApiextensionsV1().CustomResourceDefinitions().Get(ctx, "cowboys.wildwest.dev", metav1.GetOptions{})
		if err != nil {
	...
	t.Logf("Create a cowboys APIBinding in consumer workspace %q that points to the today-cowboys export from %q but shadows a local cowboys CRD at the same time", tenantShadowCRDWorkspace, serviceProvider2Workspace)
	require.NoError(t, apply(t, ctx, tenantShadowCRDWorkspace, framework.UserConfig("user-3", rest.CopyConfig(cfg)),
		&apisv1alpha1.APIBinding{
			ObjectMeta: metav1.ObjectMeta{
				Name: "cowboys",
	...
	t.Logf("Waiting for cowboys APIBinding in consumer workspace %q to have the condition %q mentioning the conflict with the shadowing local cowboys CRD", tenantShadowCRDWorkspace, apisv1alpha1.BindingUpToDate)
	framework.Eventually(t, func() (bool, string) {
		binding, err := user3KcpClient.Cluster(tenantShadowCRDWorkspace.Path()).ApisV1alpha1().APIBindings().Get(ctx, "cowboys", metav1.GetOptions{})
		if err != nil {
			return false, fmt.Sprintf("error creating API binding: %v", err)
		}
...
`))
	...
	framework.Eventually(t, func() (bool, string) {
		var err error
		apiExport, err = user2KcpClient.Cluster(serviceProvider2Workspace.Path()).ApisV1alpha1().APIExports().Get(ctx, "today-cowboys", metav1.GetOptions{})
		if err != nil {
	...
	framework.Eventually(t, func() (success bool, reason string) {
		_, err = user2DynamicVWClient.Cluster(tenantWorkspace.Path()).Resource(schema.GroupVersionResource{Version: "v1alpha1", Resource: "sheriffs", Group: "wild.wild.west"}).List(ctx, metav1.ListOptions{})
		if err != nil {
	...
	_, err = user2DynamicVWClient.Cluster(tenantShadowCRDWorkspace.Path()).Resource(schema.GroupVersionResource{Version: "v1alpha1", Resource: "cowboys", Group: "wildwest.dev"}).List(ctx, metav1.ListOptions{})
...
func apply(t *testing.T, ctx context.Context, workspace logicalcluster.Name, cfg *rest.Config, manifests ...any) error {
	t.Helper()
	discoveryClient, err := kcpkubernetesclientset.NewForConfig(cfg)
	require.NoError(t, err)
	mapper := restmapper.NewDeferredDiscoveryRESTMapper(memory.NewMemCacheClient(discoveryClient.Cluster(workspace.Path()).Discovery()))
	...
	for _, manifest := range manifests {
		...
		if mapping.Scope.Name() == meta.RESTScopeNameNamespace {
			...
			dynamicResource = dynamicClient.Cluster(workspace.Path()).Resource(mapping.Resource).Namespace(obj.GetNamespace())
		} else {
			t.Logf(`applying %q workspace %q name %q`, gvk.String(), workspace.String(), obj.GetName())
			dynamicResource = dynamicClient.Cluster(workspace.Path()).Resource(mapping.Resource)
		}
