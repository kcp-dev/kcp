...
func TestInitializingWorkspacesVirtualWorkspaceAccess(t *testing.T) {
	...
	for initializer, watcher := range watchers {
		...
		case evt := <-watcher.ResultChan():
			...
			require.Empty(t, cmp.Diff(evt.Object, ws), "got incorrect object in watch stream for initializer %s", initializer)
	...
	for initializer, watcher := range watchers {
		for {
			...
			case evt := <-watcher.ResultChan():
				...
				ws.ResourceVersion = ""
				evt.Object.(*tenancyv1alpha1.ClusterWorkspace).ResourceVersion = ""
				require.Empty(t, cmp.Diff(evt.Object, ws), "got incorrect object in watch stream for initializer %s", initializer)
			case <-time.Tick(wait.ForeverTestTimeout):
