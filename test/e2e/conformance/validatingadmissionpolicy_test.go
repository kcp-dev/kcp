...
func TestValidatingAdmissionPolicyCrossWorkspaceAPIBinding(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		_, err := cowbyClusterClient.Cluster(targetPath).WildwestV1alpha1().Cowboys("default").Create(ctx, &badCowboy, metav1.CreateOptions{})
		if err != nil {
			if errors.IsInvalid(err) {
				t.Logf("Error: %v", err)
				if strings.Contains(err.Error(), "failed expression: object.spec.intent != 'bad'") {
	...
	_, err = cowbyClusterClient.Cluster(targetPath).WildwestV1alpha1().Cowboys("default").Create(ctx, &goodCowboy, metav1.CreateOptions{})
	require.NoError(t, err)
}
