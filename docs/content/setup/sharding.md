...
**Important**: Without additional engineering effort at the code level, sharding in kcp is fundamentally a **capacity concept**, not a **high-availability concept**.
...
Partitions combined with `APIExportEndpointSlice` resources provide a mechanism to distribute API access across shards, enabling continued operation even when the provider's home shard is unavailable.
...
1. **Understand that sharding = capacity**, not automatic HA
