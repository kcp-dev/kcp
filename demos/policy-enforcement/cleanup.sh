...
# TMC Policy Enforcement Demo Cleanup Script
...
print_header() {
    echo -e "${BOLD}${PURPLE}${CLEAN} TMC Policy Enforcement Demo Cleanup${NC}"
...
cleanup_kind_clusters() {
    ...
    local clusters=("policy-kcp" "policy-dev" "policy-staging" "policy-prod")
...
cleanup_docker_containers() {
    ...
    local stale_containers=$(docker ps -a --filter "label=io.x-k8s.kind.cluster" --filter "name=policy-" --format "{{.Names}}" 2>/dev/null || true)
...
verify_cleanup() {
    ...
    local remaining_clusters=$(kind get clusters 2>/dev/null | grep -E "^policy-" || true)
...
show_cleanup_summary() {
    echo ""
    echo -e "${BOLD}${GREEN}${CHECK} Policy Enforcement Demo Cleanup Complete!${NC}"
    echo ""
    echo -e "${CYAN}Resources Cleaned Up:${NC}"
    echo -e "  • Kind clusters: policy-kcp, policy-dev, policy-staging, policy-prod"
    ...
    echo -e "  • All policy configurations are preserved in the manifests"
...
confirm_cleanup() {
    echo -e "${BOLD}${YELLOW}${WARNING} This will remove all policy enforcement demo resources!${NC}"
    echo ""
    echo -e "${CYAN}The following will be cleaned up:${NC}"
    echo -e "  • All Kind clusters (policy-kcp, policy-dev, policy-staging, policy-prod)"
    echo -e "  • Kubeconfig files and directories"
    echo -e "  • Demo log files"
    ...
    echo -e "  • Policy manifest templates"
...
if [[ "${1:-}" == "help" || "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    echo "TMC Policy Enforcement Demo Cleanup Script"
    echo ""
    echo "Removes all demo resources and clusters created by the policy enforcement demo."
    echo ""
    echo "Usage: $0 [--force]"
    echo ""
    echo "Options:"
    ...
    echo "  • Kind clusters (policy-kcp, policy-dev, policy-staging, policy-prod)"
    ...
    echo "  • Policy manifest templates"
