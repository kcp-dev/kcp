...
# TMC Hello World Demo Cleanup Script
# Removes all resources created by the hello-world demo
...
usage() {
    echo "TMC Hello World Demo Cleanup"
    echo ""
...
cleanup_demo_workloads() {
    print_info "Removing demo workloads..."
    ...
    if ! kind get clusters | grep -q "^$EAST_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete deployments,services -l demo=hello-world 2>/dev/null || true
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete pods -l demo=hello-world 2>/dev/null || true
    fi
    ...
    if ! kind get clusters | grep -q "^$WEST_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete deployments,services -l demo=hello-world 2>/dev/null || true
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete pods -l demo=hello-world 2>/dev/null || true
    fi
    ...
    if ! kind get clusters | grep -q "^$KCP_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$KCP_CLUSTER_NAME" delete namespace kcp-system 2>/dev/null || true
        kubectl --context "kind-$KCP_CLUSTER_NAME" delete configmaps -l demo=hello-world 2>/dev/null || true
    fi
...
cleanup_clusters() {
    ...
    if [[ $clusters_removed -eq 0 ]]; then
        print_info "No hello-world demo clusters found"
    else
...
cleanup_local_files() {
    ...
    if [[ -d "$DEMO_DIR/manifests" ]]; then
        rm -f "$DEMO_DIR/manifests/mock-kcp.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/sync-targets.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/east-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/west-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/hello-east.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/hello-west.yaml" 2>/dev/null || true
...
main() {
    ...
    echo -e "${BLUE}🧹 TMC Hello World Demo Cleanup${NC}"
    echo ""
    ...
    echo ""
    print_success "Hello World demo cleanup completed!"
