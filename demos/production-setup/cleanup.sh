...
# TMC Production Setup Demo Cleanup Script
# Removes all resources created by the production-setup demo
...
usage() {
    echo "TMC Production Setup Demo Cleanup"
    echo ""
...
cleanup_demo_workloads() {
    print_info "Removing production setup demo workloads..."
    ...
    if ! kind get clusters | grep -q "^$EAST_CLUSTER_NAME$"; then
        ...
        if [[ -f "$KUBECONFIG" ]]; then
            kubectl delete namespace production-workloads 2>/dev/null || true
            kubectl delete namespace kcp-syncer 2>/dev/null || true
            kubectl delete pods -l demo=$DEMO_NAME --all-namespaces 2>/dev/null || true
        fi
    fi
    ...
    if ! kind get clusters | grep -q "^$KCP_CLUSTER_NAME$"; then
        ...
        if [[ -f "$KUBECONFIG" ]]; then
            kubectl delete namespace kcp-system 2>/dev/null || true
            kubectl delete configmaps -l demo=$DEMO_NAME --all-namespaces 2>/dev/null || true
        fi
    fi
    
    if ! kind get clusters | grep -q "^$MONITOR_CLUSTER_NAME$"; then
        print_warning "Monitor cluster not found, skipping monitor cleanup"
    else
        print_info "Cleaning up monitoring resources..."
        export KUBECONFIG="$DEMO_DIR/kubeconfigs/monitor-cluster.kubeconfig"
        if [[ -f "$KUBECONFIG" ]]; then
            kubectl delete deployment prometheus grafana 2>/dev/null || true
            kubectl delete service prometheus grafana 2>/dev/null || true
            kubectl delete configmap prometheus-config grafana-datasources 2>/dev/null || true
...
cleanup_clusters() {
    ...
    if kind get clusters | grep -q "^$MONITOR_CLUSTER_NAME$"; then
        print_info "Removing monitor cluster..."
        if kind delete cluster --name "$MONITOR_CLUSTER_NAME" 2>/dev/null; then
            print_success "Monitor cluster removed"
            clusters_removed=$((clusters_removed + 1))
        else
            print_error "Failed to remove monitor cluster"
        fi
    else
        print_info "Monitor cluster not found"
    fi
    
    if [[ $clusters_removed -eq 0 ]]; then
        print_info "No production setup demo clusters found"
    else
...
cleanup_local_files() {
    ...
    if [[ -d "$DEMO_DIR/manifests" ]]; then
        rm -f "$DEMO_DIR/manifests/production-kcp.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/east-production-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/west-production-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/east-production-workload.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/west-production-workload.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/prometheus.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/grafana.yaml" 2>/dev/null || true
        print_success "Generated manifests removed"
    fi
    
    # Remove monitoring configs
    if [[ -d "$DEMO_DIR/monitoring" ]]; then
        rm -f "$DEMO_DIR/monitoring/prometheus-config.yaml" 2>/dev/null || true
        print_success "Monitoring configs removed"
    fi
...
verify_cleanup() {
    ...
    if kind get clusters | grep -E "^($KCP_CLUSTER_NAME|$EAST_CLUSTER_NAME|$WEST_CLUSTER_NAME|$MONITOR_CLUSTER_NAME)$"; then
        print_warning "Some demo clusters still exist"
        issues=$((issues + 1))
    fi
    
    # Check for Docker containers
    if docker ps -a --format "table {{.Names}}" | grep -E "$KCP_CLUSTER_NAME|$EAST_CLUSTER_NAME|$WEST_CLUSTER_NAME|$MONITOR_CLUSTER_NAME"; then
    ...
    if [[ $issues -eq 0 ]]; then
        ...
        if [[ "$FORCE_CLEANUP" == "true" ]]; then
            ...
            docker rm -f $(docker ps -aq --filter "name=$MONITOR_CLUSTER_NAME") 2>/dev/null || true
        fi
    fi
}
...
main() {
    ...
    echo -e "${BLUE}🧹 TMC Production Setup Demo Cleanup${NC}"
    echo ""
    ...
    echo ""
    print_success "Production Setup demo cleanup completed!"
    ...
    if [[ "$cleanup_type" == "demo-only" ]]; then
        ...
        echo "• Monitor: kubectl --context kind-$MONITOR_CLUSTER_NAME"
        echo ""
