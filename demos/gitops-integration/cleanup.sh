...
# TMC GitOps Integration Demo - Cleanup Script
...
print_header() {
    echo -e "${BOLD}${BLUE}================================================${NC}"
    echo -e "${BOLD}${BLUE}🧹 TMC GitOps Integration Demo Cleanup${NC}"
    echo -e "${BOLD}${BLUE}================================================${NC}"
    echo ""
}

print_usage() {
    echo "TMC GitOps Integration Demo - Cleanup Script"
    echo ""
    ...
    echo "  • Kind clusters (gitops-kcp, gitops-dev, gitops-staging, gitops-prod)"
    echo "  • Generated kubeconfig files"
    echo "  • Git repositories created for demo"
...
cleanup_demo_resources() {
    ...
    local contexts=("kind-gitops-kcp" "kind-gitops-dev" "kind-gitops-staging" "kind-gitops-prod")
    ...
    for context in "${contexts[@]}"; do
        ...
        # Remove ArgoCD resources (only on KCP cluster)
        if [[ "$context" == "kind-gitops-kcp" ]]; then
            echo -e "${CYAN}  Removing ArgoCD components...${NC}"
            kubectl --context "$context" delete namespace argocd --ignore-not-found=true &>/dev/null || [[ "$force_flag" == "true" ]]
        fi
        
        # Remove deployments
        local deployments=$(kubectl --context "$context" get deployments --no-headers -o name 2>/dev/null | grep -E "(demo-webapp|kcp-syncer|argocd)" || true)
        ...
        # Remove services
        local services=$(kubectl --context "$context" get services --no-headers -o name 2>/dev/null | grep -E "(demo-webapp|argocd)" || true)
        if [[ -n "$services" ]]; then
        ...
        # Remove secrets (ArgoCD cluster secrets)
        if [[ "$context" == "kind-gitops-kcp" ]]; then
            local secrets=$(kubectl --context "$context" get secrets --no-headers -o name 2>/dev/null | grep -E "(cluster-secret)" || true)
            if [[ -n "$secrets" ]]; then
                echo -e "${CYAN}  Removing cluster secrets...${NC}"
                echo "$secrets" | xargs -r kubectl --context "$context" delete --ignore-not-found=true &>/dev/null || [[ "$force_flag" == "true" ]]
            fi
        fi
        ...
        kubectl --context "$context" delete clusterrolebinding --ignore-not-found=true \
            argocd-server argocd-application-controller kcp-syncer &>/dev/null || [[ "$force_flag" == "true" ]]
        ...
        kubectl --context "$context" delete serviceaccount --ignore-not-found=true \
            argocd-server argocd-repo-server argocd-application-controller kcp-syncer &>/dev/null || [[ "$force_flag" == "true" ]]
...
cleanup_clusters() {
    ...
    local clusters=("gitops-kcp" "gitops-dev" "gitops-staging" "gitops-prod")
    local force_flag="${1:-false}"
...
cleanup_files() {
    ...
    if [[ -d "$GIT_REPOS_DIR" ]]; then
        echo -e "${CYAN}Removing git repositories...${NC}"
        rm -rf "$GIT_REPOS_DIR"
        echo -e "${GREEN}${CHECK} Removed git repositories${NC}"
    fi
...
main() {
    ...
    if [[ "$demo_only" == "true" ]]; then
        ...
        if [[ "$force_cleanup" != "true" ]]; then
            echo -e "${YELLOW}This will remove all demo resources, clusters, and git repositories.${NC}"
