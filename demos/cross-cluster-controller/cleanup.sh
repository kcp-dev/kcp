...
# TMC Cross-Cluster Controller Demo Cleanup Script
# Removes all resources created by the cross-cluster-controller demo
...
usage() {
    echo "TMC Cross-Cluster Controller Demo Cleanup"
    echo ""
...
cleanup_demo_workloads() {
    print_info "Removing cross-cluster controller demo workloads..."
    ...
    if ! kind get clusters | grep -q "^$EAST_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete taskqueues --all 2>/dev/null || true
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete deployments,services -l demo=$DEMO_NAME 2>/dev/null || true
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete pods -l demo=$DEMO_NAME 2>/dev/null || true
        kubectl --context "kind-$EAST_CLUSTER_NAME" delete crd taskqueues.demo.tmc.io 2>/dev/null || true
    fi
    ...
    if ! kind get clusters | grep -q "^$WEST_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete taskqueues --all 2>/dev/null || true
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete deployments,services -l demo=$DEMO_NAME 2>/dev/null || true
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete pods -l demo=$DEMO_NAME 2>/dev/null || true
        kubectl --context "kind-$WEST_CLUSTER_NAME" delete crd taskqueues.demo.tmc.io 2>/dev/null || true
    fi
    ...
    if ! kind get clusters | grep -q "^$KCP_CLUSTER_NAME$"; then
        ...
        kubectl --context "kind-$KCP_CLUSTER_NAME" delete taskqueues --all 2>/dev/null || true
        kubectl --context "kind-$KCP_CLUSTER_NAME" delete crd taskqueues.demo.tmc.io 2>/dev/null || true
        kubectl --context "kind-$KCP_CLUSTER_NAME" delete configmaps -l demo=$DEMO_NAME 2>/dev/null || true
    fi
...
cleanup_clusters() {
    ...
    if [[ $clusters_removed -eq 0 ]]; then
        print_info "No cross-cluster controller demo clusters found"
    else
...
cleanup_local_files() {
    ...
    if [[ -d "$DEMO_DIR/manifests" ]]; then
        rm -f "$DEMO_DIR/manifests/taskqueue-crd.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/taskqueue-controller.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/east-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/west-syncer.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/east-taskqueue.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/west-taskqueue.yaml" 2>/dev/null || true
        rm -f "$DEMO_DIR/manifests/global-taskqueue.yaml" 2>/dev/null || true
...
main() {
    ...
    echo -e "${BLUE}🧹 TMC Cross-Cluster Controller Demo Cleanup${NC}"
    echo ""
    ...
    echo ""
    print_success "Cross-Cluster Controller demo cleanup completed!"
