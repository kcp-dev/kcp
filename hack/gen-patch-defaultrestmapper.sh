...
git diff --quiet -- "${DEFAULTRESTMAPPER_PATCH_FILEPATH}" || {
    echo "Warning: ${DEFAULTRESTMAPPER_PATCH_FILEPATH} was modified."
    echo "Warning: Please inspect the changes before continuing. Run:"
    printf "\n\tgit diff -- %s\n\n" "${DEFAULTRESTMAPPER_PATCH_FILEPATH}"
    exit 0
}
